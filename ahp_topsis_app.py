...
def main():
    ...
    if st.button("Calculate Rankings"):
        ...
        df = pd.DataFrame({
            ...
            'Score': np.round(closeness, 4),
            'Rank': closeness.rank(ascending=False).astype(int)
        }).sort_values('Rank')

        # Display styled table
        st.markdown("### 📊 Ranking of Alternatives")
        st.dataframe(
            df.style
        ...
        st.markdown("### ⚖️ Criteria Weights (AHP)")
        weight_df = pd.DataFrame({
            'Criterion': criteria_list,
            'Weight': np.round(weights, 4)
        })
        st.dataframe(weight_df.style.set_properties(**{'text-align': 'center'}))

        # Consistency ratio with visual feedback
        st.markdown("### 🧮 Consistency Check")
        consistency_message = f"""
        <div style="padding: 15px; border-radius: 10px; background-color: {'#d1e7dd' if cr <= 0.1 else '#f8d7da'}; color: {'#0f5132' if cr <= 0.1 else '#842029'};">
            <b>Consistency Ratio:</b> {cr:.3f} <br>
            {'✅ The pairwise comparisons are consistent.' if cr <= 0.1 else '⚠️ The comparisons are inconsistent. Please revise them.'}
        </div>
        """
        st.markdown(consistency_message, unsafe_allow_html=True)
